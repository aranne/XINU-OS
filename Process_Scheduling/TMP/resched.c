...
{	
	...
	if (currpid != NULLPROC) {
		int prev_quantum;
		if (optr->pquantum >= QUANTUM) {
			prev_quantum = QUANTUM;
		} else {
			prev_quantum = optr->pquantum;
		}
		optr->pquantum -= (prev_quantum - time);
	}

	if (optr->pstate == PRCURR) {
		optr->pstate = PRREADY;
		insert(currpid, rdyhead, optr->pprevprio);
	}
	...
	/* get the max goodness process */
	currpid = getbestproc(rdytail);
	nptr = &proctab[currpid];
	nptr->pstate = PRCURR; /* mark it currently running	*/

#ifdef RTCLOCK
    /* reset preemption counter	*/
	if (nptr->pquantum >= QUANTUM) {
		preempt = QUANTUM;
	} else {
		preempt = nptr->pquantum;
	}
#endif
